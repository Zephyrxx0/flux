    ...
    <div className="space-y-6" data-testid="risk-report-sections">
      ...
      <section className="space-y-2" aria-label="critical-zones">
        ...
        {report.criticalZones.length > 0 ? (
          ...
          <div className="bg-black/40 p-4 rounded-lg border border-white/5 text-muted-foreground text-sm italic">
      ...
      <section className="space-y-2" aria-label="staffing-recommendations">
        ...
        {report.staffingRecommendations.length > 0 ? (
          <ul className="space-y-2 text-sm text-foreground/80">
          ...
          <div className="bg-black/40 p-4 rounded-lg border border-white/5 text-muted-foreground text-sm italic">
            No staffing recommendations required at this time.
          </div>
        )}
      </section>
      ...
      <section className="space-y-2 border-t border-white/10 pt-4" aria-label="evidence-block">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-black/40 p-3 rounded-lg border border-white/5 flex flex-col gap-1 overflow-hidden">
            <span className="text-[10px] uppercase tracking-widest text-muted-foreground">Run Hash</span>
            <span className="text-xs font-mono text-white truncate" title={report.evidence.runDeterministicHash}>
              {report.evidence.runDeterministicHash}
            </span>
          </div>
          <div className="bg-black/40 p-3 rounded-lg border border-white/5 flex flex-col gap-1">
            <span className="text-[10px] uppercase tracking-widest text-muted-foreground">Rows Analyzed</span>
            <span className="text-xs font-mono text-white">
              {report.evidence.rowsAnalyzed}
            </span>
          </div>
        </div>
      </section>
